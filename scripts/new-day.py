...
def create_problem_file(day: str, problem_number: int) -> None:
    ...
    with open(problem_file_path, "w") as fp:
        fp.write(PROBLEM_TEMPLATE.replace("{{day}}", day).replace("{{problem_number}}", str(problem_number)))

def create_test_file(day: str, problem_number: int) -> None:
    test_file_path = get_test_dir(day=day) / Path(f"test_problem_{problem_number}_day_{day}.py")
    with open(test_file_path, "w") as fp:
        fp.write(TEST_TEMPLATE.replace("{{day}}", day).replace("{{problem_number}}", str(problem_number)))

def create_day_folder(day: str) -> None:
    day_dir = get_day_dir(day=day)
    day_dir.mkdir(parents=True, exist_ok=False)

    with open(day_dir / "__init__.py", "w"): pass
...
def create_test_folder(day: str) -> None:
    ...
    with open(get_test_dir(day=day) / "__init__.py", "w"): pass
...
def new_day(day: Annotated[str, typer.Argument()]):
    ...
    print(f"[green]Successfully created new day folder structure for Day {day}![/green]")
